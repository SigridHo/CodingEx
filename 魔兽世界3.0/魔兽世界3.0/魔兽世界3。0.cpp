...
// weapon part
/* class sword's function: Power, NoPower, GetPower*/
class sword
{
	int Power;
public:
...
/*class arrow's function: UseArrow, GetPower, GetRestTimes, NoRestTimes*/
class arrow
{
	int RestTimes;
	int Power;
public:
	arrow(int a):Power(a), RestTimes(3){};
	int UseArrow() 
	{
		RestTimes--;
		return Power;
	...
	int GetPower() {return Power;}
	int GetRestTimes() {return RestTimes;}
	bool NoRestTimes()
	{
		if(RestTimes ==  0) return true;
		else return false;
	}
};
...
public:
	...
	int Hp;
	int Flag;   // 0 means no falg, >0 blue, <0 red
	Warrior* state[2];
	void build(int x);
	int GetHp() {return Hp;}
...
void City::ChangeFlag()
{
	...
	if(temp1[Number] == 0 || (temp1[Number] == -1 && temp2[Number] == 0) || temp1[Number] == 2)
	{
		...
		else 
		{
			Flag--;
			Color = 0;
			if(Flag == -2)printf("%.3d:40 red flag raised in city %d\n", Hour, Number);
		}
	}
	if(temp1[Number] == 1 || (temp1[Number] == -1 && temp2[Number] == 1) || temp1[Number] == 3)
	{
		...
		else
		{
			Flag++;
			Color = 1;
			if(Flag == 2)printf("%.3d:40 blue flag raised in city %d\n", Hour, Number);
		}
	}
}
...
void City::YELL()
{
	if(temp1[Number] != 10 && (state[0] != NULL)&& (state[0]->Type == DRAGON))
	{
		if(Color == 0 || (Color == -1 && Number % 2 == 1))
		{
	...
	if(temp1[Number] != 10&& (state[1] != NULL) && (state[1]->Type == DRAGON))
	{
		if(Color == 1 || (Color == -1 && Number % 2 == 0))
		{
...
int City:: ATTACK() 
{
	...
	bool wolf = false;
	if((state[0] != NULL) && (state[1] != NULL) && (state[0]->Hp <= 0) && (state[1]->Hp <= 0)) 
	{state[0] = NULL; state[1] = NULL; return -1;}   // only one warrior and it has been killed by arrow
	/* only one warrior and it has been killed by arrow*/
...
void LIONRUNAWAY()
{
	for(int i = 0; i <= CityNumber + 1; ++i)
	{
		for(int j = 0; j < 2; ++j)
		{
			if(i == 0)
			{
				if((citys[i].state[j] != NULL) && (citys[i].state[j]->Color == 0) && (citys[i].state[j]->LIONESCAPE()))
				{
					printf("%.3d:05 red lion %d ran away\n", Hour, citys[i].state[j]->Number);
					citys[i].state[j] = NULL;
				}
			}
			else if(i == CityNumber + 1)
			{
				if((citys[i].state[j] != NULL) && (citys[i].state[j]->Color == 1) && (citys[i].state[j]->LIONESCAPE()))
				{
...
void FREETAKE()
{
	...
	{
		if(citys[i].state[0] != NULL && citys[i].state[0]->Hp > 0)
		{
			if(citys[i].state[1] == NULL || citys[i].state[1]->Hp == 0)
			{
			...
			if(citys[i].state[0] != NULL && citys[i].state[0]->Arrow != NULL)
			{
				if(citys[i + 1].state[1] != NULL)
				{
					...
					if(citys[i + 1].state[1]->Hp <= 0)
					{
						...
						if(citys[i + 1].state[0] != NULL) citys[i + 1].state[1]->Hp = 0;
						else citys[i + 1].state[1] = NULL;
					}
					else
					{
						printf("%.3d:35 red %s %d shot\n", Hour, WarriorName[citys[i].state[0]->Type],citys[i].state[0]->Number);
					}
				}
			}
		}
		else if(i == CityNumber)
		{
			if(citys[i].state[1] != NULL && citys[i].state[1]->Arrow != NULL)
			{
				if(citys[i - 1].state[0] != NULL)
				{
					...
					if(citys[i - 1].state[0]->Hp <= 0)
					{
						...
						if(citys[i - 1].state[1] != NULL) citys[i - 1].state[0]->Hp = 0;
						else citys[i -1].state[0] = NULL;
					}
					else
					{
						printf("%.3d:35 blue %s %d shot\n", Hour, WarriorName[citys[i].state[1]->Type],citys[i].state[1]->Number);
					}
				}
			}
		}
...
void CITYPRIZE()
{
	for(int i = CityNumber; i >= 1; --i)
	{
		...
		if(citys[i].Color == 0 || (citys[i].Color == - 1 && citys[i].Number % 2 == 1))
		{
	...
	for(int i = 1; i <= CityNumber; ++i)
	{
		...
		if(citys[i].Color == 1 || (citys[i].Color == - 1 && citys[i].Number % 2 == 0))
		{
		...
		if(HeadquarterLife[1] >= 8 && bluewin)
		{
			prize[i] = 1;
		}
	}
...
int main() {
	...
	for(int i = 1; i <= t; ++ i)
	{
		 ...
		 while(1)
		 {
			 ...
			 for(int j = 1; j <= CityNumber; ++j)
			 {
				 temp1[j] = citys[j].ATTACK();
				 if(temp1[j] == 1 ||temp1[j] == 3 || (temp1[j] == -1 && temp2[j] == 1)) 
				 {
					 if(temp1[j] != 3)printf("%.3d:40 red %s %d was killed in city %d\n", Hour, WarriorName[citys[j].state[0]->Type], citys[j].state[0]->Number, j);
					 citys[j].state[0] = NULL;
					 citys[j].DragonCheck();
					 citys[j].YELL();
				 ...
				 {
					 if(temp1[j] != 2) printf("%.3d:40 blue %s %d was killed in city %d\n", Hour, WarriorName[citys[j].state[1]->Type], citys[j].state[1]->Number, j);
					 citys[j].state[1] = NULL;
					 citys[j].DragonCheck();
					 citys[j].YELL();
				 ...
				 if(temp1[j] == -1)
				 {
					 ...
					 if(temp1[j] == 1 || (temp1[j] == -1 && temp2[j] == 1)) 
					 {
					 ...
					 if(temp1[j] == 0 || (temp1[j] == -1 && temp2[j] == 0)) 
					 {
